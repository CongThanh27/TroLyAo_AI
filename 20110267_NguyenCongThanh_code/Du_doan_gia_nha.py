...
from Giaodien_chinh import Bot_khongnghe,Bot_langnghe,Bot_noi,Hienthithongtinngheduoc
...
processed_train_set_val = full_pipeline.fit_transform(train_set) # Xử lý số liệu
...
def print_search_result(grid_search, model_name = ""): 
    ...
    cv1 = KFold(n_splits=5,shuffle=True,random_state=37) # cv data generator
...
if run_new_search:        
        ...
        grid_search = GridSearchCV(model, param_grid, cv=cv1, scoring='neg_mean_squared_error', return_train_score=True, 
...
def chuyen_doi(text):
    ...
    if 2:
        if 'mét' in text:
            key.append(lay_so_lieu(test_set, text, 'mét'))
        else:
            key.append(np.nan)          
    if 3:
        if "đông nam" in text:
    ...
    if 4:
        if 'phòng' in text:
            key.append(lay_so_lieu(test_set, text, 'phòng'))
        else:
            key.append(np.nan)  
    if 5:
        if 'toilet' in text:
            key.append(lay_so_lieu(test_set, text, 'toilet'))
        else:
            key.append(np.nan)  
    if 6:
...
#%%
def get_audio(time_limi=5):
    r = sr.Recognizer()
    with sr.Microphone() as source:
        print("Tôi: ", end='')
        audio = r.listen(source, phrase_time_limit=time_limi)
        try:
...
def index_text(list_text, tu): # vị trí của từ cần kiếm ở đâu trong list
    i = 0
...
def lay_so_lieu(test_set, text, tu):
    list_text = text.split()
    vi_tri_tu = index_text(list_text, tu)
    ...
    try:
       ...
       # speak("Có lẽ anh đã nói nhầm từ " + so + " " + list_text[vi_tri_tu] + " rồi")
        Bot_noi(4,f"Có lẽ bạn đã nói nhầm từ {so} {list_text[vi_tri_tu]} rồi")
        return np.nan
#%%
def doan_gia_nha(test_set):
    Bot_langnghe(4,"...")
    thongtin = get_audio(10).lower()
    Hienthithongtinngheduoc(3,thongtin)
    feature = chuyen_doi(thongtin)
    ...
    gia_du_doan = "Anja nghĩ giá của nó đâu khoảng " + (str(gia_du_doan).strip('[]')).replace(".", "") + "000000 đồng, rất thích hợp để mua"
    #speak(gia_du_doan)
    Bot_noi(6,gia_du_doan)
